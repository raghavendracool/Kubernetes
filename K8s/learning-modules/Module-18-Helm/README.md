# Module 18 — Helm — Package Kubernetes Applications

## 1. What Will We Learn?

**Outcome:** Convert the progressive app into a reusable Helm chart.

## 2. The 3 Whys

1. **WHY #1 — Why Helm? Many YAML files become difficult to repeat.**
2. **WHY #2 — Why values? Environment settings should not duplicate templates.**
3. **WHY #3 — Why releases? Install/upgrade/rollback need application history.**
...
Plain YAML is ingredients; Helm chart is a reusable recipe with values.
...
Package Deployment/Service/Ingress into a Helm chart.

If this module contains a progressive `stage/` folder:

```bash
bash scripts/load-module.sh 18
...
```bash
helm template student-web ./student-web
```

```bash
helm install student-web ./student-web -n student-app --create-namespace
```

```bash
helm upgrade student-web ./student-web -n student-app
```

```bash
helm history student-web -n student-app
```
...
- Render/lint in CI.
- Do not store production secrets in values.
...
1. Chart vs release?
2. What is values.yaml?
3. Why helm template in CI?
