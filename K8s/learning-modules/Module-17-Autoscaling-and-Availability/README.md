# Module 17 — Autoscaling and Availability — HPA, PDB and Node Scaling

## 1. What Will We Learn?

**Outcome:** Scale Pods and protect capacity during voluntary disruption.

## 2. The 3 Whys

1. **WHY #1 — Why HPA? Demand changes.**
2. **WHY #2 — Why PDB? Maintenance should preserve minimum availability.**
3. **WHY #3 — Why node scaling? More Pods require compute capacity.**
...
HPA adds checkout counters, node autoscaler expands the store, PDB keeps minimum counters open.
...
Apply HPA/PDB, generate load and watch replicas.

If this module contains a progressive `stage/` folder:

```bash
bash scripts/load-module.sh 17
...
- Set requests before HPA.
- Use PDB with multiple replicas.
- Separate Pod and node autoscaling.
